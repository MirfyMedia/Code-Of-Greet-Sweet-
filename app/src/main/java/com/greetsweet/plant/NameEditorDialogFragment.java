...
public class NameEditorDialogFragment extends DialogFragment {

    public static final String TAG = NameEditorDialogFragment.class.getSimpleName();
    ...
    public static NameEditorDialogFragment show(@NonNull AppCompatActivity appCompatActivity,
                                                @NonNull String inputText,
                                                @ColorInt int colorCode) {
        Bundle args = new Bundle();
        args.putString(EXTRA_INPUT_TEXT, inputText);
        args.putInt(EXTRA_COLOR_CODE, colorCode);
        NameEditorDialogFragment fragment = new NameEditorDialogFragment();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.new_addname_editor, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mAddTextEditText.setText(PhotoApp.getInstance().getstoreSharedpref().getSharedValue(Cons.sendname));
...
*/
        ...
        if(PhotoApp.getInstance().getstoreSharedpref().getSharedValue("writingtype").equals("roboto"))
        {
            mAddTextEditText.setText(PhotoApp.getInstance().getstoreSharedpref().getSharedValue(Cons.sendname));
            mAddTextEditText.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), roboto));
            facetype=Typeface.createFromAsset(getActivity().getAssets(),roboto);
        }
        else if(PhotoApp.getInstance().getstoreSharedpref().getSharedValue("writingtype").equals(Cons.font_lucida))
        {
            mAddTextEditText.setText(PhotoApp.getInstance().getstoreSharedpref().getSharedValue(Cons.sendname));
            mAddTextEditText.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), Luci));
            facetype=Typeface.createFromAsset(getActivity().getAssets(),Luci);
        }
        else if(PhotoApp.getInstance().getstoreSharedpref().getSharedValue("writingtype").equals(Cons.font_adlery))
        {
            mAddTextEditText.setText(PhotoApp.getInstance().getstoreSharedpref().getSharedValue(Cons.sendname));
            mAddTextEditText.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), aldery));
            facetype=Typeface.createFromAsset(getActivity().getAssets(),aldery);
        }
        else if(PhotoApp.getInstance().getstoreSharedpref().getSharedValue("writingtype").equals(Cons.font_together))
        {
            mAddTextEditText.setText(PhotoApp.getInstance().getstoreSharedpref().getSharedValue(Cons.sendname));
            mAddTextEditText.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), together));
            facetype=Typeface.createFromAsset(getActivity().getAssets(),together);
        }
        else if(PhotoApp.getInstance().getstoreSharedpref().getSharedValue("writingtype").equals(Cons.font_blanch))
        {
            mAddTextEditText.setText(PhotoApp.getInstance().getstoreSharedpref().getSharedValue(Cons.sendname));
            mAddTextEditText.setTypeface(Typeface.createFromAsset(getActivity().getAssets(), blanch));
            facetype=Typeface.createFromAsset(getActivity().getAssets(),blanch);
